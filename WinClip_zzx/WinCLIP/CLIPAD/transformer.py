...
class WindowVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # self.use_pix_multi_scale_zzx = True
    ...
    def generate_window_masks_uniAD(self, feature_size=15, neighbor_size_list=[3, 15]):
        """
        Generate a square mask for the sequence. The masked positions are filled with float('-inf').
        Unmasked positions are filled with float(0.0).
        """
        mask_list = []
        for neighbor_size in neighbor_size_list:
            h = w = feature_size
            hm = wm = neighbor_size
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.use_pix_multi_scale == True: 
            ...
            # x = x.permute(1, 0, 2)
            x = self.ln_pre(x)
            mask_list = self.generate_window_masks_uniAD()
            
            x = x.permute(1, 0, 2)  # NLD -> LND
            tokens_list = []
            for mask in mask_list:
                y = self.transformer(x, attn_mask=mask)
                y = y.permute(1, 0, 2)  # LND -> NLD
                # pooled, tokens = self._global_pool(y)
                tokens = self.ln_post(y)
