...
from model import LinearLayer, TextAdapter, Adapter, ReverseProjection
...
def test(args):
    ...
    reverse_projection = ReverseProjection(model_configs['vision_cfg']['width'], model_configs['embed_dim'],
                              len(features_list), args.model, model).to(device)
    # dataset
    ...
    for items in test_dataloader:
        ...
        with torch.autograd.set_detect_anomaly(True):           # easier debug
            with torch.no_grad():
                ...
                text_features_attn = reverse_projection(text_features)
                image_features, patch_tokens = model.encode_image(image, features_list, text_feature=text_features_attn)
            ...
            normal_layer = 6
            abnormal_layer = 7
            normal_patch_tokens = resize_tokens(patch_tokens_ln[normal_layer])
            abnormal_patch_tokens = resize_tokens(patch_tokens_ln[abnormal_layer])
            if args.adapter:
                with torch.no_grad():
                    tokens = adapters[cls_name[0]](tokens, is_test=True)
            
            normal_patch_tokens /= normal_patch_tokens.norm(dim=-1, keepdim=True)
            abnormal_patch_tokens /= abnormal_patch_tokens.norm(dim=-1, keepdim=True)

            normality_score = normal_patch_tokens @ text_features[0,:,0]
            abnormality_score = abnormal_patch_tokens @ text_features[0,:,1]
            
            anomaly_map = torch.cat((normality_score.unsqueeze(-1), abnormality_score.unsqueeze(-1)), dim=-1)
