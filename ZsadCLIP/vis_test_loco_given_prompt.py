...
from dataset import VisaDataset, MVTecDataset, MvTecLocoDataset
...
def test(args):
    ...
    if dataset_name == 'mvtec':
        ...
        test_data = MvTecLocoDataset(root=dataset_dir, transform=data_transform, target_transform=transform, mode='test')
        ...
        with torch.autograd.set_detect_anomaly(True):           # easier debug
            ...
            for layer in range(len(patch_tokens_ln)):
                ...
                anomaly_map = torch.nn.functional.interpolate(
                                anomaly_map, (items['img_ori_size'][1].item(), items['img_ori_size'][0].item()), mode='bilinear')
            ...
            # interpolate the anomaly map to the original size
            ...
            # save anomaly maps
            result_save_dir = os.path.join(save_path, 'imgs', cls_name[0], 'test', cls)
            if not os.path.exists(result_save_dir):
                os.makedirs(result_save_dir, exist_ok=True)
            img_nm = filename.split('.')[0] + '.tiff'
            tifffile.imwrite(os.path.join(result_save_dir, img_nm), anomaly_map[0])
...
if __name__ == '__main__':
    ...
    # paths
    parser.add_argument("--data_path", type=str, default="/home/zhaoxiang/dataset/mvtec_loco_anomaly_detection", help="path to test dataset")
    parser.add_argument("--save_path", type=str, default='/home/zhaoxiang/CLIP_AD/ZsadCLIP/results/loco', help='path to save results')
    ...
    # model
    parser.add_argument("--dataset", type=str, default='loco', choices=['mvtec', 'visa', 'loco'], help="test dataset")
    ...
    parser.add_argument("--image_size", type=list, default=[240, 240], help="image size")
