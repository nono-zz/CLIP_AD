...
def test(args):
    ...
    if args.training:
        ...
        with torch.no_grad():
            ...
            fnn_dataset = RandomWordPrompt(num_sentences, model, tokenizer, device, args)
        ...
        optimizer = torch.optim.AdamW(fnn.parameters(), lr=learning_rate, weight_decay=weight_decay)
        ...
        for epoch in range(epochs):
            for batch_id, (batch_prompts, batch_labels) in enumerate(fnn_dataloader):
                ...
                batch_prompts = batch_prompts.view(-1, batch_prompts.shape[-1])
        ...
        experiment_name = model_string + '_' + dataset_string + '_' + 'random_word'
        model_dict_path = experiment_name+'.pth'
        torch.save(fnn.state_dict(), model_dict_path)
        ...
        fnn = FNN(input_dim=640, hidden_dim=640, dropout_prob=0.5)
        fnn.load_state_dict(torch.load(model_dict_path))
        fnn.to(device)
